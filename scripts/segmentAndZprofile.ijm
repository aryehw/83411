 ...
 *   
 *   Also, the results table that is generated by the particle analyzer can be saved, and tehn  used in R or Excel 
 ...
 *   if the 0% exposure is first. It would be possible to check the stack to se which slice is strongest, 
 ...
 *   
 *   To do:	1. Prompt the use for the spatial calibration information, and incorporate it into the image. 
...
// Just in case an ROI was selected, make sure it is deselected so tha twe work on the entire image.
run("Select None");
...
Array.reverse(ypoints);	// program assumes that the input was ordered strongestto weakest illumination
